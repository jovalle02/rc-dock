    ...
    render() {
        ...
        for (let [id, tabCache] of this._cache) {
            ...
            items.push({
                ...
                children: (React.createElement(DockTabPane, { cacheId: id, cached: tabCache.cached, active: isActive, prefixCls: "dock" }, tabCache.tabContent)),
                forceRender: tabCache.cached !== false,
                closable: tabCache.data.closable,
                disabled: false,
            });
        }
        return (React.createElement(Tabs, { prefixCls: "dock", more: { icon: moreIcon }, renderTabBar: this.renderTabBar, activeKey: activeId, onChange: this.onTabChange, popupClassName: classNames(groupClassNames(group)), items: items }));
    }
}
